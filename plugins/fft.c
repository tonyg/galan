...
#define GENERATOR_CLASS_NAME	"fftw"
#define GENERATOR_CLASS_PATH	"Misc/FFT"
...
PRIVATE void evt_input_handler(Generator *g, AEvent *event) {
  ...
  if( event->d.array.len != data->lastN ) {
      ...
      data->plan = rfftw_create_plan( data->lastN, FFTW_REAL_TO_COMPLEX, FFTW_ESTIMATE );
  }
...
PUBLIC void init_plugin_fft(void) {
  setup_class();
}
