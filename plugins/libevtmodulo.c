...
#define GENERATOR_CLASS_NAME	"evtmodulo"
#define GENERATOR_CLASS_PATH	"Events/Modulo"
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->factor = objectstore_item_get_double(item, "evtmodulo_factor", 1);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "evtmodulo_factor", data->factor);
}
...
PRIVATE void evt_input_handler(Generator *g, AEvent *event) {
    ...
    switch( event->kind ) {
	case AE_NUMBER:
	    event->d.number = (int) event->d.number % (int) ((Data *) g->data)->factor;
	    gen_send_events(g, EVT_OUTPUT, -1, event);
	    break;
	case AE_NUMARRAY:
	    for( i=0; i<event->d.array.len; i++ )
		event->d.array.numbers[i] = (int) event->d.number % (int) ((Data *) g->data)->factor;
	    
	    gen_send_events(g, EVT_OUTPUT, -1, event);
	    break;
	default:
	    g_warning( "evtmodulo does not know this eventkind\n" );
	    break;
    }
}

PRIVATE void evt_factor_handler(Generator *g, AEvent *event) {
    if( event->d.number != 0.0 )
