...
#define GENERATOR_CLASS_NAME	"evtabs"
#define GENERATOR_CLASS_PATH	"Events/Abs"
...
PRIVATE void evt_value_handler(Generator *g, AEvent *event) {
  ...
  event->d.number = abs(event->d.number);
...
PRIVATE void setup_class(void) {
  ...
  gen_configure_event_output(k, EVT_OUTPUT, "Abs");

  gencomp_register_generatorclass(k, FALSE, GENERATOR_CLASS_PATH, NULL, NULL);
}

PUBLIC void init_plugin_evtabs(void) {
  setup_class();
}
