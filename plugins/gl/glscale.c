...
PRIVATE gboolean render_function(Generator *g ) {
    ...
    glPushMatrix();
    glScalef( data->tx, data->ty, data->tz );
    gen_render_gl( g, 0, -1 );
    glPopMatrix();
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->tx = objectstore_item_get_double(item, "trans_x", 1);
  data->ty = objectstore_item_get_double(item, "trans_y", 1);
  data->tz = objectstore_item_get_double(item, "trans_z", 1);
}
...
PRIVATE void setup_class(void) {
  ...
  gen_configure_event_input( k, EVT_TRANSX, "ScaleX", evt_trans_x_handler );
  gen_configure_event_input( k, EVT_TRANSY, "ScaleY", evt_trans_y_handler );
  gen_configure_event_input( k, EVT_TRANSZ, "ScaleZ", evt_trans_z_handler );
  
  
  gencomp_register_generatorclass(k, FALSE, "GL/scale", NULL, NULL);
}
