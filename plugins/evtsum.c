...
#define GENERATOR_CLASS_NAME	"evtsum"
#define GENERATOR_CLASS_PATH	"Events/Integrate"
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->addend = objectstore_item_get_double(item, "evtsum_addend", 0);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "evtsum_addend", data->addend);
}
...
PRIVATE ControlDescriptor controls[] = {
  ...
  { CONTROL_KIND_KNOB, "value", -100,100,0.1,0.01, 0,TRUE, TRUE,EVT_ADDEND,
...
PRIVATE void setup_class(void) {
  ...
  gen_configure_event_input(k, EVT_ADDEND, "Set", evt_addend_handler);
