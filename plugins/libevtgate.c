...
#define GENERATOR_CLASS_NAME	"evtgate"
#define GENERATOR_CLASS_PATH	"Events/Gate"
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->comp = objectstore_item_get_double(item, "gate_comp", 0);
  data->open = objectstore_item_get_integer(item, "gate_open", 0);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "gate_comp", data->comp);
  objectstore_item_set_integer(item, "gate_open", data->open);
}
...
PRIVATE ControlDescriptor controls[] = {
  ...
  { CONTROL_KIND_KNOB, "comp", 0,32,1,1, 0,TRUE, TRUE,EVT_COMP,
...
PRIVATE void setup_class(void) {
  ...
  gen_configure_event_input(k, EVT_TOCOMP, "ToCompare", evt_tocomp_handler);
