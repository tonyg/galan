...
#define GENERATOR_CLASS_NAME	"evtdiv"
#define GENERATOR_CLASS_PATH	"Events/Divide"
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->factor = objectstore_item_get_double(item, "evtdiv_factor", 1);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "evtdiv_factor", data->factor);
}
...
PRIVATE void evt_input_handler(Generator *g, AEvent *event) {
	...
	case AE_NUMARRAY:
	    ...
	    break;
	default:
	    g_warning( "evtdiv does not know this eventkind\n" );
	    break;
    }
}
