...
#define GENERATOR_CLASS_NAME	"evtadd"
#define GENERATOR_CLASS_PATH	"Events/Sum"
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->addend = objectstore_item_get_double(item, "evtadd_addend", 0);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "evtadd_addend", data->addend);
}
...
PRIVATE void evt_input_handler(Generator *g, AEvent *event) {
	...
	case AE_NUMARRAY:
		...
		event->d.array.numbers[i] += ((Data *) g->data)->addend;
	    
	    gen_send_events(g, EVT_OUTPUT, -1, event);
	    break;
	default:
	    g_warning( "evtsum does not know this eventkind\n" );
	    break;
    }
}
...
PRIVATE void setup_class(void) {
  ...
  gen_configure_event_input(k, EVT_ADDEND, "Addend", evt_addend_handler);
