...
#define GENERATOR_CLASS_NAME	"midievt_cc"
#define GENERATOR_CLASS_PATH	"MIDI Events/CC"

#define EVT_VALUE		0
#define EVT_CONTROLER		1
...
PRIVATE void unpickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  ...
  data->controler = objectstore_item_get_double(item, "controler", 0);
  data->channel = objectstore_item_get_double(item, "channel", 0);
}

PRIVATE void pickle_instance(Generator *g, ObjectStoreItem *item, ObjectStore *db) {
  Data *data = g->data;
  objectstore_item_set_double(item, "controler", data->controler);
...
PRIVATE void evt_value_handler(Generator *g, AEvent *event) {
    ...
    switch( event->kind ) {
	case AE_NUMBER:
	    val = ((char) event->d.number) & 0x7f;
	    event->kind = AE_MIDIEVENT;
	    event->d.midiev.len = 3;
	    event->d.midiev.midistring[0] = 0xB0 | (data->channel & 0x0f);
	    event->d.midiev.midistring[1] = data->controler;
	    event->d.midiev.midistring[2] = val; 
...
PRIVATE void evt_controler_handler(Generator *g, AEvent *event) {
    ((Data *) g->data)->controler = (char) event->d.number & 0x7f;
}
...
PRIVATE void setup_class(void) {
    ...
    gen_configure_event_input(k, EVT_CONTROLER, "Controler", evt_controler_handler);
